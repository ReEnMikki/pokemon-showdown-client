...
Storage.bg = {
  ...
  load: function (bgUrl, bgid, hues) {
    ...
    if (!hues) {
      switch (bgid) {
        ...
        case "ocean":
          ...
          attrib =
            '<a href="https://quanyails.deviantart.com/art/Sunrise-Ocean-402667154" target="_blank" class="subtle">"Sunrise Ocean" <small>background by Quanyails</small></a>';
          break;
        case "waterfall":
          hues = [
          ...
          attrib =
            '<a href="https://yilx.deviantart.com/art/Irie-372292729" target="_blank" class="subtle">"Irie" <small>background by Samuel Teo</small></a>';
          break;
        case "shaymin":
          hues = [
          ...
          attrib =
            '<a href="http://cargocollective.com/bluep" target="_blank" class="subtle">"Shaymin" <small>background by Daniel Kong</small></a>';
          break;
        case "charizards":
          hues = [
          ...
          attrib =
            '<a href="https://seiryuuden.deviantart.com/art/The-Ultimate-Mega-Showdown-Charizards-414587079" target="_blank" class="subtle">"Charizards" <small>background by Jessica Valencia</small></a>';
          break;
        case "itu":
          hues = [
            "318.87640449438203,35.177865612648226%",
		  ...
		  attrib =
            '<a href="../fx/client-bg-itu.jpg" target="_blank" class="subtle">"Itu" <small>background by Nekki</small></a>';
          break;
        case "ambient":
          hues = [
            "318.87640449438203,35.177865612648226%",
		  ...
		  attrib =
            '<a href="../fx/client-bg-ambient.jpg" target="_blank" class="subtle">"Ambient" <small>background by Nekki</small></a>';
          break;
        case "arena":
          hues = [
            "318.87640449438203,35.177865612648226%",
		  ...
		  attrib =
            '<a href="../fx/client-bg-arena.jpg" target="_blank" class="subtle">"Arena" <small>background by Nekki</small></a>';
          break;
        case "void":
          hues = [
            "318.87640449438203,35.177865612648226%",
		  ...
		  attrib =
            '<a href="../fx/client-bg-void.jpg" target="_blank" class="subtle">"Void" <small>background by Nekki</small></a>';
          break;
        case "psday":
          hues = [
            "24.705882352941174,25.37313432835821%",
            "260.4651162790697,59.44700460829492%",
            "165.3191489361702,46.07843137254901%",
            "16.363636363636367,42.63565891472869%",
            "259.04761904761904,34.05405405405405%",
            "24.705882352941174,25.37313432835821%",
          ];
          attrib =
            "Pok&eacute;mon Showdown Day background <small>by LifeisDANK</small>";
          break;
        case "digimon":
          hues = [
    ...
    if (attrib)
      attrib =
        '<small style="display:block;padding-bottom:4px">' +
        attrib +
        "</small>";
  ...
  loadHues: function (hues) {
    ...
    for (var i = 0; i < 5; i++) {
      ...
      cssBuf +=
        "body .button.mainmenu" +
        n +
        ":hover { background: linear-gradient(to bottom,  hsl(" +
        hs +
        ",62%),  hsl(" +
        hs +
        ",42%)); border-color: hsl(" +
      ...
      cssBuf +=
        "body .button.mainmenu" +
        n +
        ":active { background: linear-gradient(to bottom,  hsl(" +
        hs +
        ",42%),  hsl(" +
        hs +
        ",58%)); border-color: hsl(" +
...
try {
  if (window.localStorage) {
    Storage.prefs.data =
      JSON.parse(localStorage.getItem("showdown_prefs")) || {};
  }
} catch (e) {}
...
var updatePrefs = function () {
  ...
  if (Storage.prefs("nogif") !== undefined) {
    ...
    Storage.whenAppLoaded(function () {
      app.addPopupMessage(
        'Your version of Chrome has a bug that makes animated GIFs freeze games sometimes, so certain animations have been disabled. Only some people have the problem, so you can experiment and enable them in the Options menu setting "Disable GIFs for Chrome 64 bug".'
      );
    });
  }

  var colorSchemeQuerySupported =
    window.matchMedia &&
    ...
    case "a":
      ...
      if (!window.nodewebkit) {
        ...
        $.get = function (uri, data, callback, type) {
          ...
          Storage.postCrossOriginMessage(
            "R" + JSON.stringify([uri, data, idx, type])
          );
        };
        $.post = function (uri, data, callback, type) {
          ...
          Storage.crossOriginRequests[idx] = callback;
          Storage.postCrossOriginMessage(
            "S" + JSON.stringify([uri, data, idx, type])
          );
        };
      }
      break;
  ...
  try {
    ...
    Storage.whenAppLoaded(function (app) {
      app.addPopup(Popup, {
        type: "modal",
        htmlMessage:
          'Your teams are corrupt and could not be loaded. :( We may be able to recover a team from this data:<br /><textarea rows="10" cols="60">' +
          BattleLog.escapeHTML(buffer) +
          "</textarea>",
      });
    });
  }
...
Storage.packAllTeams = function (teams) {
  return teams
    .map(function (team) {
      return (
        (team.format
          ? "" + team.format + (team.capacity === 24 ? "-box]" : "]")
          : "") +
...
Storage.fastUnpackTeam = function (buf) {
  ...
  while (true) {
    ...
    set.ability =
      species.abilities && ["", "0", "1", "H", "S"].includes(ability)
        ? species.abilities[ability] || "!!!ERROR!!!"
        : ability;
    i = j + 1;
...
Storage.packedTeamIcons = function (buf) {
  ...
  return this.packedTeamNames(buf)
    .map(function (species) {
      return (
        '<span class="picon" style="' +
        Dex.getPokemonIcon(species) +
        ';float:left;overflow:visible"><span style="font-size:0px">' +
        toID(species) +
        "</span></span>"
      );
    })
  ...
  for (var i = 0; i < text.length; i++) {
    ...
    if (line === "" || line === "---") {
      ...
      if (teams.length && typeof teams[teams.length - 1].team !== "string") {
        teams[teams.length - 1].team = Storage.packTeam(
...
Storage.exportTeam = function (team, gen, hidestats) {
  ...
  for (var i = 0; i < team.length; i++) {
    ...
    if (
      typeof curSet.dynamaxLevel === "number" &&
      curSet.dynamaxLevel !== 10 &&
      !isNaN(curSet.dynamaxLevel)
    ) {
    ...
    if (gen === 9) {
      text +=
        "Tera Type: " +
        (curSet.teraType || Dex.species.get(curSet.species).types[0]) +
        "  \n";
    }
    if (!hidestats) {
      var first = true;
...
Storage.initDirectory2 = function () {
  ...
  fs.mkdir(self.documentsDir + "My Games", function () {
    fs.mkdir(self.documentsDir + "My Games/Pokemon Showdown", function () {
      fs.stat(
        self.documentsDir + "My Games/Pokemon Showdown",
        function (err, stats) {
          if (err) return;
...
Storage.nwLoadNextBatch = function (files, offset, dirOffset) {
  if (window.app) {
    window.app.addPopupMessage(
      "Loading " +
        files.length +
        " teams (Teams load slowly if you have over 2000 teams)"
    );
  }
  var i;
...
Storage.nwSaveTeam = function (team) {
  ...
  if (team.format)
    filename =
      "[" + team.format + (team.capacity === 24 ? "-box] " : "] ") + filename;
  filename = filename.trim().replace(/[\\\/]+/g, "");
  if (team.folder)
    filename = "" + team.folder.replace(/[\\\/]+/g, "") + "/" + filename;
  ...
  fs.writeFile(
    this.dir + "Teams/" + filename,
    Storage.exportTeam(team.team, team.gen).replace(/\n/g, "\r\n"),
    function () {}
  );
};
...
Storage.nwLogChat = function (roomid, line) {
  ...
  if (!this.chatLogStreams[roomid]) {
    this.chatLogStreams[roomid] = fs.createWriteStream(
      this.dir + "Logs/" + chatLogFdMonth + "/" + roomid + ".txt",
      { flags: "a" }
    );
